...
@onready var model_a = preload("res://Assets/3D Models/Dice3DWith9.glb")
@onready var model_b = preload("res://Assets/3D Models/Dice3DWith7and9.glb")
...
func change_3d_model(die):
	if die.possible_values.find(6) != -1: 
		if die.possible_values.find(7) != -1: 
			...
			die.add_child(new_model)
			new_model.name = "DiceModel"
	elif die.possible_values.find(7) != -1:
